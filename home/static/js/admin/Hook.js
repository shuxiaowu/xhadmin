...
CodeInfoDlg.collectData = function () {
	this.set('hook_id').set('hook_name').set('description').set('sortid').set('create_time');
};
...
CodeInfoDlg.add = function () {
	 ...
	 var tip = '添加';
	 var ajax = new $ax(Feng.ctxPath + "/Hook/add", function (data) {
	 	if ('00' === data.status) {
	 		Feng.success(tip + "成功" );
...
CodeInfoDlg.update = function () {
	 ...
	 var status = $("input[name = 'status']:checked").val();
	 var tip = '修改';
	 var ajax = new $ax(Feng.ctxPath + "/Hook/update", function (data) {
	 	if ('00' === data.status) {
	 		Feng.success(tip + "成功" );
	 		window.parent.CodeGoods.table.refresh();
	 		CodeInfoDlg.close();
