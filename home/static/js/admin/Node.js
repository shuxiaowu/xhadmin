...
CodeInfoDlg.collectData = function () {
	this.set('id').set('title').set('pid').set('val').set('sortid').set('icon');
};

CodeInfoDlg.icon = function () {
		var index = layer.open({type: 2,title: '设置图标',area: ['800px', '500px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Base/icon/field/icon'});
...
CodeInfoDlg.add = function () {
	 ...
	 var tip = '添加';
	 var ajax = new $ax(Feng.ctxPath + "/Node/add", function (data) {
	 	if ('00' === data.status) {
	 		Feng.success(tip + "成功" );
...
CodeInfoDlg.update = function () {
	 ...
	 var ajax = new $ax(Feng.ctxPath + "/Node/update", function (data) {
	 	if ('00' === data.status) {
	 		Feng.success(tip + "成功" );
	 		window.parent.CodeGoods.table.refresh();
	 		CodeInfoDlg.close();
