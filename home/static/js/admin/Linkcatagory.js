...
CodeInfoDlg.collectData = function () {
	this.set('catagory_id').set('title').set('sortid');
};
...
CodeInfoDlg.add = function () {
	 ...
	 var tip = '添加';
	 var ajax = new $ax(Feng.ctxPath + "/Linkcatagory/add", function (data) {
	 	if ('00' === data.status) {
	 		Feng.success(tip + "成功" );
...
CodeInfoDlg.update = function () {
	 ...
	 var tip = '修改';
	 var ajax = new $ax(Feng.ctxPath + "/Linkcatagory/update", function (data) {
	 	if ('00' === data.status) {
	 		Feng.success(tip + "成功" );
	 		window.parent.CodeGoods.table.refresh();
	 		CodeInfoDlg.close();
