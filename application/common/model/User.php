...
class User extends Model
{
	...
	/*关联查询*/
	 public function loadRelateList($where = [],$field='*',$limit,$orderby){
		...
		try{
			$list = db($this->tableName)->field($field)->alias('a')->join('group b','a.group_id=b.group_id',"LEFT")->where($where)->limit($limit)->order($orderby)->select();
			
			$count = db($this->tableName)->field($field)->alias('a')->join('group b','a.group_id=b.group_id',"LEFT")->where($where)->count();
