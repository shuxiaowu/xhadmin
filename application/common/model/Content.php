...
class Content extends Model
{
	...
	protected $tableName = 'content';   //数据表名
	...
	/*关联查询*/
	 public function loadRelateList($where = [],$field='*',$limit,$orderby){
		...
		try{
			$list = db($this->tableName)->field($field)->alias('a')->join('catagory b','a.class_id=b.class_id',"LEFT")->where($where)->limit($limit)->order($orderby)->select();
			
			$count = db($this->tableName)->field($field)->alias('a')->join('catagory b','a.class_id=b.class_id',"LEFT")->where($where)->limit($limit)->order($orderby)->count();
		}catch(\Exception $e){
			throw new \Exception($e->getMessage());
		}
